...
function normalize(raw: RawAX[], deviceId: string): Snapshot {
  const nodes: AXNode[] = raw.map((r, i) => normalizeNode(r, i));
...
function normalizeNode(r: RawAX, i: number): AXNode {
  return {
    id: r.AXUniqueId || `n${i}`,
    type: prettyType(r),
    role: r.role,
    ...
    customActions: Array.isArray(r.custom_actions) ? r.custom_actions : [],
    contentRequired: Boolean(r.content_required),
    frame: {
      x: r.frame.x,
      y: r.frame.y,
      w: r.frame.width,
